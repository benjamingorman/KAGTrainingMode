...
void onInit(CBlob@ this)
{
    ...
    this.addCommandID("begin attack");
    this.addCommandID("release attack");
    this.addCommandID("finish attack");
}
...
{
    if (this.isOnGround() && this.getPlayer() !is null) {
        this.getPlayer().set_Vec2f("last ground pos", this.getPosition());
        this.getPlayer().set_bool("use last ground pos", true);
    }
    ...
    if (this.getPlayer() is null) {
        log("onTick", "pressed_a1 = " + pressed_a1);
        log("onTick", "swordState = " + swordState);
        log("onTick", "key just released: " + this.isKeyJustReleased(key_action1));
    }
    */
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		...
		{
            SendReleaseAttackCommand(this, knight.swordTimer);
			knight.swordTimer = 0;
	...
	{
        //log("onTick", "block 3. knight timer=" + (this.get_u32("knight_timer") <= getGameTime()));
		knight.state = KnightStates::normal;
	}
...
void SendBeginAttackCommand(CBlob@ this) {
    if (!getNet().isServer()) return;
    //log("SendBeginAttackCommand", "Called");
    this.SendCommand(this.getCommandID("begin attack"));
}

void SendReleaseAttackCommand(CBlob@ this, u8 swordTimer) {
    if (!getNet().isServer()) return;
    //log("SendReleaseAttackCommand", "Called");
    CBitStream params;
    params.write_u8(swordTimer);
    this.SendCommand(this.getCommandID("release attack"), params);
}

void SendFinishAttackCommand(CBlob@ this) {
    if (!getNet().isServer()) return;
    //log("SendFinishAttackCommand", "Called");
    this.SendCommand(this.getCommandID("finish attack"));
}
