...
shared class SandboxSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (canSpawnPlayer(p_info))
		{
			...
			p_info.blob_name = "knight"; //hard-set the respawn blob
            Vec2f spawnLoc;
            if (player.exists("last ground pos") && player.get_bool("use last ground pos")) {
                // set this in knightlogic
                spawnLoc = player.get_Vec2f("last ground pos");
            }
            else {
                spawnLoc = getSpawnLocation(p_info);
            }

			CBlob@ playerBlob = SpawnPlayerIntoWorld(spawnLoc, p_info);
            if (player.exists("last team")) {
                s8 lastTeam = player.get_s8("last team");
                playerBlob.server_setTeamNum(lastTeam);
            }
...
void SetupBases()
{
    ...
    if (map !is null)
    {
        ...
        if (!getMap().getMarkers("blue main spawn", respawnPositions))
        {
            ...
            respawnPos.y -= 16.0f;
            server_CreateBlob(base_name, 0, respawnPos);
        }
        else
        {
        ...
        if (!getMap().getMarkers("red main spawn", respawnPositions))
        {
            ...
            respawnPos.y -= 16.0f;
            server_CreateBlob(base_name, 1, respawnPos);
        }
        else
        {
            for (uint i = 0; i < respawnPositions.length; i++)
...
void Reset(CRules@ this)
{
    ...
    for (int i=0; i < getPlayerCount(); i++) {
        ...
        player.set_bool("use last ground pos", false);
    }
}
